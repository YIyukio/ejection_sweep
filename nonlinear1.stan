functions{
  vector npcurve(real b0, real b1, real b2, vector speed) {
    return (b0 + b1 * (1-exp(-b2 / b1 * speed)));
  }
  vector rpcurve(real b0, real b1, vector speed) {
    return (b0 + b1 * speed);
  }
  real npcurve_real(real b0, real b1, real b2, real speed) {
    return (b0 + b1 * (1-exp(-b2 / b1 * speed)));
  }
...
transformed parameters {
  ...
  fitted_np100 = npcurve(offset_np, pmax, alpha, speed);
...
generated quantities {
  ...
  yfit_np_100 = npcurve(offset_np, pmax, alpha, speed_predict);
  ...
  for(n in 1:N) {
    log_lik[n] = normal_lpdf(net100[n] | npcurve_real(offset_np, pmax, alpha, speed[n]), sigma_np);
  }
  for(n in (N+1):(2*N)) {
