...
model {
  ...
  sigma ~ cauchy(0, 2.5);
  gross100 ~ lognormal(log(fitted), sigma);
}
generated quantities {
  vector[M] expected100;
  vector[M] expected;
  vector[N] log_lik;
  expected100 = modelcurve(offset, pmax, alpha, beta, speed_predict);
  expected = expected100/100;
  for(n in 1:N) {
    log_lik[n] = lognormal_lpdf(log(gross100[n]) | log(fitted[n]), sigma);
  }
}
