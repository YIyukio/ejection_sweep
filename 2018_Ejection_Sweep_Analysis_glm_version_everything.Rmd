...
**Important: 02_16_41_H was removed because the mean velocity is 1/10 
...
Removing the 02_16_41_H dataset.

```{r}
out = 
  out %>% 
  filter(!(Hz == 2 & Depth == 16 & Distance == 41 &
             str_detect(Density, "H")))
```
...
run_analysis = function(HSIZE, out, testrun = TRUE) {
  ...
  if(testrun) {
    out2 = out %>% mutate(data = map(data, . %>% slice(1000:1999)))
  }
  dset =
    out2 %>% ungroup() %>% 
    mutate(Position = 
...
data_0 = run_analysis(HSIZE = 0, out = out, testrun = TRUE)
data_1 = run_analysis(HSIZE = 1, out = out, testrun = TRUE)
data_2 = run_analysis(HSIZE = 2, out = out, testrun = TRUE)
data_3 = run_analysis(HSIZE = 3, out = out, testrun = TRUE)
data_4 = run_analysis(HSIZE = 4, out = out, testrun = TRUE)
```
